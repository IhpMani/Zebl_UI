...
export class PhysicianListComponent implements OnInit, OnDestroy {
  ...
  columns: Array<{
    ...
    { key: 'phyID', label: 'Physician ID', visible: true, filterValue: '' },
    { key: 'phyDateTimeCreated', label: 'Date Created', visible: true, filterValue: '' },
    { key: 'phyFullNameCC', label: 'Full Name', visible: true, filterValue: '' },
    { key: 'phyFirstName', label: 'First Name', visible: false, filterValue: '' },
    { key: 'phyLastName', label: 'Last Name', visible: false, filterValue: '' },
    { key: 'phyNPI', label: 'NPI', visible: true, filterValue: '' },
    { key: 'phyType', label: 'Type', visible: true, filterValue: '' },
  ...
  loadAvailableColumns(): void {
    this.physicianApiService.getAvailableColumns()
  ...
  loadPhysicians(page: number, pageSize: number): void {
    this.loading = true;
    this.error = null;
    ...
    if (this.columnValueFilters['phyInactive'] && this.columnValueFilters['phyInactive'].size > 0) {
      const inactiveSet = this.columnValueFilters['phyInactive'];
    ...
    if (this.columnValueFilters['phyType'] && this.columnValueFilters['phyType'].size > 0) {
      const typeSet = this.columnValueFilters['phyType'];
      const typeArray = Array.from(typeSet).filter(s => s !== '(Blank)');
      if (typeArray.length === 1) {
        filters.type = typeArray[0];
      }
    }
    ...
    this.physicianApiService.getPhysicians(page, pageSize, filters)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response: PhysiciansApiResponse) => {
        ...
        error: (err) => {
          if (err.status === 0) {
            ...
            this.error = 'Failed to load physicians. Please check if the backend is running.';
  ...
  onPageChange(page: number): void {
    this.loadPhysicians(page, this.pageSize);
  }

  onPageSizeChange(pageSize: number): void {
    this.pageSize = pageSize;
    this.loadPhysicians(1, pageSize);
  }

  onRowClick(physician: PhysicianListItem): void {
    // Navigate to physician details if needed
  }
  ...
  onFilterChange(): void {
    this.loadPhysicians(1, this.pageSize);
  }
  ...
  getCellValue(physician: PhysicianListItem, key: string): any {
    const columnDefinition = this.columns.find(c => c.key === key);
    if (columnDefinition?.isRelatedColumn && physician.additionalColumns) {
      return physician.additionalColumns[key];
    }
    return (physician as any)[key];
  }
  ...
  private getAllUniqueValuesForColumn(columnKey: string): string[] {
    ...
    for (const physician of this.physicians) {
      const v = this.getCellValue(physician, columnKey);
